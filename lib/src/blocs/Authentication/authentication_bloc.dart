...
import 'package:mangoHub/src/models/UImodels/FileUploadResponse.dart';
...
class AuthenticationBloc extends Bloc<AuthenticationEvent, AuthenticationState> {
  ...
  Stream<AuthenticationState> mapEventToState(
    ...
    if(event is PasswordResetRequest){
      yield* _passwordResetRequest(event);
    }
    if(event is PasswordReset){
      yield* _passwordReset(event);
    }
    // if(event is ImageUpload){
    //   yield* _imageUpload(event);
    // }

  }

  // Stream<AuthenticationState> _imageUpload(ImageUpload event) async* {
  //   final String apiUrl = mainPath+"/file/upload?type=Profile";
  //   String token = await _repository.readData('token');
  //   try {
  //     FileUploadResponse fileUploadResponse = new FileUploadResponse();
  //     // var stream = new http.ByteStream(DelegatingStream.typed(event.image.openRead()));
  //
  //       //create multipart request for POST or PATCH method
  //     var uri = Uri.parse(apiUrl);
  //     var request = http.MultipartRequest("POST", uri);
  //       //add text fields
  //       request.fields["text_field"] = "profile image";
  //       //create multipart using filepath, string or bytes
  //       var pic = await http.MultipartFile.fromPath("file_field", event.image.path);
  ...
  //       fileUploadResponse = FileUploadResponse.fromJson(jsonDecode(value));
  //       _repository.addValue('fileUploadResponse', jsonEncode(fileUploadResponse));
  //     });
  //
  //     if(response.statusCode == 200){
  //       yield ImageUploadSuccess(fileUploadResponse);
  //     }
  //     if (response.statusCode >= 400 ){
  //       yield ImageUploadFailed(fileUploadResponse);
  //     }
  //   } catch (e) {
  //     String errorObject = e.toString().split(':')[1];
  //     yield ImageUploadFailedException(errorObject);
  //   }
  // }
  ...
  Stream<AuthenticationState> _getLoginUserDetails(GetLoginUserDetails event) async* {
    ...
    try{
      final response = await http.post(mainPath+"/authentication/login",
          body: {
            "login_user_name": event.authentication.login.loginUserName,
  ...
  Stream<AuthenticationState> _passwordResetRequest(PasswordResetRequest event)async*{

    try{
      final response = await http.post(mainPath+"/authentication/password-reset-request",
        headers: <String, String>{
          "Content-Type": "application/json; charset=UTF-8",
        },
        body: jsonEncode(<String, dynamic>{
      ...
      if (response.statusCode == 200) {
        var passwordResetRequestResponse = jsonDecode(response.body);
      }
      if (response.statusCode >= 400) {
        var errorResponse = jsonDecode(response.body);
      }
  ...
  Stream<AuthenticationState> _passwordReset(PasswordReset event)async*{

    try{
      final response = await http.post(mainPath+"/authentication/password-reset?${event.token}",
        body: jsonEncode(<String, dynamic>{
      ...
      if (response.statusCode == 200) {
        var passwordResetResponse = jsonDecode(response.body);
      }
      if (response.statusCode >= 400) {
        var errorResponse = jsonDecode(response.body);
      }
